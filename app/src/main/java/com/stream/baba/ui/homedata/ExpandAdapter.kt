...
class ExpandAdapter(private val context: Context,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.home_result_grid_expanded, parent, false)
        return ViewHolder(view)
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val item = itemList[position]
        holder.bindData(item!!,clickCallback,viewModel)
    }
    ...
    fun addItem(list: List<Data>) {
        val insertPosition = itemList.size // Determine the position where items will be inserted
        itemList.addAll(list)
        notifyItemRangeInserted(insertPosition, list.size) // Notify that items were inserted
    }

    fun removeItem(list: List<Data>) {
        itemList.removeAll(list)
        notifyItemRangeRemoved(itemList.size,list.size)
    }
    ...
    inner class ViewHolder constructor(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindData(data: Data, clickCallback: () -> Unit, viewModel: HomeViewModel) {
